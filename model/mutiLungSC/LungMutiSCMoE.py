...
from attn.MEFM import MEFM

from torchinfo import summary

import gc

class LungMutiSCMoE(nn.Module):
    def __init__(self, 
                 attribute_classes,
        ...
        self.conv = nn.Conv2d(self.base_dim, self.base_dim, 1)
        
        self.mefm = MEFM(in_channels=self.base_dim, num_features=self.num_shared_experts)
        self.tmfm = TMFM(in_channels=self.base_dim, out_channels=self.base_dim)
    ...
    def forward(self, x, return_gates=False):
        ...
        shared_features_merge = self.mefm(*shared_features)
        
        outputs = {}
        gate_dict = {}
        
        for task in self.tasks:
            # 任务特征提取 [B, task_dim, 7, 7]
            task_features = [expert(x) for expert in self.task_expert_pools[task]]

            # 将共享特征投影到任务维度（通过1x1卷积）
            shared_to_task = [
                self.conv.to(x.device)(feat)  # [B, task_dim, 7, 7]
            ...
            # TMFM+门控权重计算
            gate_weights = self.gates[task](self.tmfm(shared_features_merge))  # [B, total_experts]
...
if __name__ == "__main__":
    ...
    # 模型初始化并移至主设备
    model = LungMutiSCMoE(attribute_classes, num_shared_experts=8,
    ...
    # 输入数据在主设备
    dummy = torch.randn(2, 3, 224, 224).to(main_device)
    ...
    #     # 前向传播
    #     out = expert(dummy)
    #     print(f"{task}专家: {out.shape} (设备: {out.device})")
