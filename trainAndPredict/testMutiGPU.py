...
if __name__ == "__main__":
    ...
    config = {
        'data_path': './dataset',
        'batch_size': 32,
        'checkpoint_path': './result/MutiExperiments/LungMutiSCMoE_42_100/checkpoints/final_model.pth',
        'result_dir': './result/MutiExperiments/LungMutiSCMoE_42_100/test_results'
    }
    # 初始化数据加载器
    ...
    model = LungMutiSCMoE(attribute_classes, num_shared_experts=4,
                 num_task_experts=2).to(main_device)
        
    model = nn.DataParallel(model.to(main_device), device_ids=device_ids)
    
    tester = Tester(config, model, test_loader, attribute_classes, main_device)
